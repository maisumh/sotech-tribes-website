...
function AppleBadge({ className = "" }: { className?: string }) {
  return (
    <span
      className={`inline-flex items-center gap-3 rounded-xl bg-black px-5 py-2.5 text-white transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-black/25 ${className}`}
    >
      ...
      <span className="text-left leading-none">
        <span className="block text-[0.6rem] font-normal tracking-wide">Download on the</span>
...
function PlayBadge({ className = "" }: { className?: string }) {
  return (
    ...
    >
      ...
      <span className="text-left leading-none">
        <span className="block text-[0.6rem] font-normal tracking-wide">GET IT ON</span>
