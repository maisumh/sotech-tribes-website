...
export function V2ChatActions({
  ...
  const [state, formAction] = useActionState(moderateV2Chat, INITIAL_STATE)
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-3">
        {!isBlocked ? (
          <form action={formAction} className="contents">
            ...
            <ActionButton intent="danger" label="Block chat" pendingLabel="Blocking…" />
          </form>
        ) : (
          <form action={formAction} className="contents">
            <input type="hidden" name="chatId" value={chatId} />
            <input type="hidden" name="op" value="unblock" />
            <ActionButton intent="primary" label="Unblock chat" pendingLabel="Unblocking…" />
          </form>
        )}
        ...
        {isReported && (
          <form action={formAction} className="contents">
            ...
            <ActionButton intent="secondary" label="Clear report" pendingLabel="Clearing…" />
          </form>
        )}
      </div>

      <p className="mt-5 pt-5 border-t border-granny/15 text-[11px] text-granny font-light italic">
        Blocking prevents both participants from exchanging new messages. Chat reports are also
        triaged in the Reports queue. All actions are logged to the admin audit trail.
      </p>
    </div>
  )
}
