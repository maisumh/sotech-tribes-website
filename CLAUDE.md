...
- `/plus` — **Identity and Tribes Plus, for Dan** (added 2026-08-23, noindex, unlinked). The follow-up to the 14 Aug touch point: the four identity models (real name -> display name -> free verification -> verification inside Plus), what a verification check costs, and the levers a paid tier could pull. Deliberately framed as options and levers rather than decisions — it is a discussion document, and it gets rewritten once the seven questions at the foot of it are settled. Links to `/costs` for vendor pricing rather than restating it. ⚠️ House register for these client pages: **no em dashes and no contractions** — match `/costs`.
...
- `src/app/robots.ts` + `src/app/sitemap.ts` are App Router metadata routes. Robots blocks `/admin`, `/api`, and the demo/preview/client pages (`/home2`, `/mvp`, `/update`, `/feedback`, `/costs`, `/plus`); sitemap lists public pages only. GSC: `sc-domain:trytribes.com` (domain property, already verified — covers www + apex).
